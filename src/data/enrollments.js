 ...
 *
 * `id` mirrors the catalog course id (src/services/courses) so
 ...
 *
 * Timestamps are generated relative to "now" at module load so the demo shows
 * believable recency without hardcoded stale dates.
 *
 * @typedef {Object} Enrollment
 * @property {number} id Catalog course id.
 * @property {"Enrolled"|"In Progress"|"Completed"} enrollmentStatus
 * @property {number} completedLessons
 * @property {string} lastAccessed ISO timestamp.
 * @property {string} [dueDate] ISO timestamp (tasks only).
 */
